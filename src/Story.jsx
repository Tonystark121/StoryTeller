...
export default function Story() {
  ...
  // To handle if not found stories.
  const [noStories, setNoStories] = useState(null);

  const [windowWidth, setWindowWidth] = useState(window.innerWidth);
  // to close the dropdown if clicked outside the option
  ...
  // to add navigation
  useEffect(() => {
    const pat = location.pathname.slice(1);
    console.log(pat);
    setSearch(pat);
  }, []);
  ...
  useEffect(() => {
    if (randomCat.length > 0) {
      ...
      if (search) {
        setCheck(search);
      } else setCheck(randomCat[random]);
    }
  }, [randomCat]);
  ...
  // taking input user topic and description function.
  ...
  // showing the options based on browsed search.
  ...
  async function handleSubmit(e) {
    ...
    if (subject && describe && selectedCategory) {
      await push(ref(database, `List/${selectedCategory}`), Data);
  ...
  // showing the browse options.
  function handleClick() {
    console.log(show3, show4);
    setShow3((prev) => !prev);
    setShow4(false);
  }
  ...
  // passing category selected from top
      ...
      onValue(
        ...
        function (snapshot) {
          ...
          if (snapshot.exists()) {
            ...
            setNoStories(`No story related to ${search.toUpperCase()} found`);
          }
        }
      );
      navigate(`/${search}`);
  ...
  // function to show top and desc. box in small screen
    ...
    <div className="flex">
      ...
      <div className="story-section">
              ...
              <div className="selectCategory">
                ...
                {show && (
                  <div className="content">
                    ...
                    {/* seeing the searching tag for post */}
                    {searchText.length === 0 ? (
        ...
        <section className="section-2">
          {/* read stories and search(looking for) */}
          <div className="section-2-head">
            {noStories && <h1>No related Stories found</h1>}
            {noStories === null && (
              <>
                {search && !selectedValue && <h1>Read stories on {search}</h1>}
                {!search && selectedValue && (
                  <h1>Read stories on {selectedValue}</h1>
                )}
                {!search && !selectedValue && <h1>Read their Stories</h1>}
              </>
            )}

            <div className="looking" ref={documentRef}>
          ...
          <div className="filter">
            {noStories === null ? (
              ...
              <h1 className="total-story">{noStories}</h1>
            )}
          </div>
